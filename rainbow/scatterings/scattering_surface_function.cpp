...
rainbow::bidirectional_scattering_surface_distribution_function::bidirectional_scattering_surface_distribution_function(
	const surface_interaction& interaction, real eta) :
	mInteraction(interaction), mEta(eta)
{
}

rainbow::separable_bidirectional_scattering_surface_distribution_function::separable_bidirectional_scattering_surface_distribution_function(
	const surface_interaction& interaction, real eta)
...
rainbow::scattering_surface_sample rainbow::separable_bidirectional_scattering_surface_distribution_function::sample(
	const std::shared_ptr<scene>& scene, const vector3& sample)
{
	...
	const auto l = 2 * sqrt(max_r * max_r - r * r);
...
rainbow::real rainbow::separable_bidirectional_scattering_surface_distribution_function::pdf(
	const surface_interaction& interaction)
{
	...
	const real projection[] = {
		sqrt(local_d.y * local_d.y + local_d.z * local_d.z),
		sqrt(local_d.z * local_d.z + local_d.x * local_d.x),
		sqrt(local_d.x * local_d.x + local_d.y * local_d.y)
	};
	...
	for (size_t channel = 0; channel < spectrum::num_samples; channel++) {
		for (auto axis = 0; axis < 3; axis++) {
			pdf = pdf + pdf_reflectance_profile(channel, projection[axis])
				* abs(local_n[axis]) * channel_pdf * axis_pdf[axis];
		}
	}
