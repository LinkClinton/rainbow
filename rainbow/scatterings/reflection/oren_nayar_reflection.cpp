...
rainbow::scatterings::oren_nayar_reflection::oren_nayar_reflection(const spectrum& reflectance, real sigma) :
...
rainbow::spectrum rainbow::scatterings::oren_nayar_reflection::evaluate(const vector3& wo, const vector3& wi) const
{
	// f(wo, wi) = R \ pi * (A + B * max(0, cos(phi_i - phi_o)) * sin(alpha) * tan(beta)
	...
	if (abs(cos_theta(wi)) > abs(cos_theta(wo))) {
		sin_alpha = sin_theta_o;
		tan_beta = sin_theta_i / abs(cos_theta(wi));
	}else {
		sin_alpha = sin_theta_i;
		tan_beta = sin_theta_o / abs(cos_theta(wo));
	}
