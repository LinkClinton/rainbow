...
rainbow::cpus::scatterings::fresnel_blend_reflection::fresnel_blend_reflection(
...
spectrum rainbow::cpus::scatterings::fresnel_blend_reflection::evaluate(const vector3& wo, const vector3& wi) const
{
	auto wh = wo + wi;
	...
	const auto diffuse = mReflectanceDiffuse * (28 / (23 * pi<real>())) *
		...
		(1 - pow_5(1 - static_cast<real>(0.5) * math::abs(cos_theta(wi)))) *
		(1 - pow_5(1 - static_cast<real>(0.5) * math::abs(cos_theta(wo))));
	...
	const auto specular = fresnel * distribution /
		(4 * math::abs(dot(wi, wh)) * max(math::abs(cos_theta(wi)), math::abs(cos_theta(wo))));

	return mScale * (specular + diffuse);
}

rainbow::cpus::scatterings::scattering_sample rainbow::cpus::scatterings::fresnel_blend_reflection::sample(
...
rainbow::core::real rainbow::cpus::scatterings::fresnel_blend_reflection::pdf(const vector3& wo, const vector3& wi) const
{
	...
	const auto sample_hemisphere_pdf = cosine_sample_hemisphere_pdf(math::abs(cos_theta(wi)));

	return static_cast<real>(0.5) * (sample_distribution_pdf + sample_hemisphere_pdf);
}

spectrum rainbow::cpus::scatterings::fresnel_blend_reflection::schlick_fresnel(real cos_theta) const noexcept
{
	return mReflectanceSpecular + (spectrum(1) - mReflectanceSpecular) * pow_5(1 - cos_theta);
}

rainbow::core::real rainbow::cpus::scatterings::fresnel_blend_reflection::pow_5(real value) const noexcept
{
