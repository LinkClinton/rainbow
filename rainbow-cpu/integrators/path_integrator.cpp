	...
	const ray& first_ray, size_t depth)
{
	...
	for (auto bounces = depth; bounces < mMaxDepth; bounces++) {
		...
		beta = beta * scattering_sample.value * math::abs(dot(scattering_sample.wi, interaction->shading_space.z())) / scattering_sample.pdf;
		...
		if (surface_properties.bssrdf != nullptr && has(scattering_sample.type, scattering_type::transmission)) {
			if (!sample_scattering_surface_function(scene, samplers, surface_properties, ray, beta, L, specular_bounce))
				break;
		}
